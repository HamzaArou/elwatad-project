...
const PropertySearch = () => {
  ...
  return <section className="relative -mt-[89.5px] z-10">
      <div className="container mx-auto px-4">
        <div style={{
        maxWidth: "1120px"
      }} className="mx-auto bg-white shadow-lg rounded-xl px-[12px] py-[16px]">
            ...
            <div className="flex flex-col gap-6 px-8">
              ...
              <div className="w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:flex lg:justify-between">
                  {cities.map(city => <button key={city.id} onClick={() => setSelectedCity(city.name)} className={cn("relative h-24 w-full lg:w-[48%] rounded-lg overflow-hidden group transition-all duration-300", selectedCity === city.name ? "ring-2 ring-gold" : "hover:ring-2 hover:ring-gold/50")}>
              ...
              <div className="w-full">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {propertyTypes.map(type => <button key={type.id} onClick={() => setSelectedType(type.name)} className={cn("py-3 px-4 rounded-lg text-center transition-all duration-300 w-full", selectedType === type.name ? "bg-gold text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-700")}>
